...
public class BluetoothLePlugin extends CordovaPlugin
{
    ...
    private final String keyCharacteristicId = "characteristicId";
    ...
    private BluetoothGattCharacteristic getCharacteristic(JSONObject obj, BluetoothGattService service)
    {
        ...
        int uuidCharacteristicId;

        try
        {
            uuidCharacteristic = UUID.fromString(uuidCharacteristicValue);
            uuidCharacteristicId = obj.getInt(keyCharacteristicId);
        }
        catch (Exception ex)
        {
            return null;
        ...
        {
            if (characteristic.getUuid().equals(uuidCharacteristic) && characteristic.getInstanceId() == uuidCharacteristicId)
            {
    ...
    private boolean testError(String message, CallbackContext callbackContext)
    {
        JSONObject returnObj = new JSONObject();

        addProperty(returnObj, keyError, errorConnect);
        addProperty(returnObj, keyMessage, message);
    ...
    private void addCharacteristic(JSONObject returnObj, BluetoothGattCharacteristic characteristic)
    {
        ...
        addProperty(returnObj, keyCharacteristicId, characteristic.getInstanceId());
    }
    ...
    private JSONObject getDiscovery(BluetoothGatt bluetoothGatt)
    {
        ...
        for (BluetoothGattService service : services)
        {
            ...
            for (BluetoothGattCharacteristic characteristic : characteristics)
            {
                ...
                addProperty(characteristicObject, keyCharacteristicId, characteristic.getInstanceId());
